...
- 10 个连续关卡，每关使用独立路线与 4～6 个经过覆盖检查的塔位，每关 3～5 个自动波次；出怪前在入口显示下一波与数字倒计时。
- 上述 4～6 塔位属于当前可玩原型；已确认下一版将改为 12～18 个潜在建造单元，并加入清除障碍、释放空间和出售转移火力。
- 豆包（快速单体）、棉棉（减速控制）、布丁（范围驱赶）三名店员。
- 普通、高速、重型三种敌人。
- 先点空塔位再从塔位旁选择店员；点已建店员显示范围和升级入口，另有三个一键道具、正式的 1/2/3 倍速、暂停和胜负结算。
